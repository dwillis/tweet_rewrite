  ...
  class Application < Sinatra::Base
    ...
    get '/show' do
      ...
      if @url.include?('nyti.ms')
        ...
        @tweets = @client.search(@url.split('?').first, :result_type => @result_type).statuses.reject{|i| i.text.include?(@item.title.split.first(5).join(' '))}.reject{|i| i.text[0..1] == 'RT'}
        tweets2 = @client.search(@tw_url.split('?').first, :result_type => @result_type).statuses.reject{|i| i.text.include?(@item.title.split.first(5).join(' '))}.reject{|i| i.text[0..1] == 'RT'}
        ...
        @item = Item.url(@url)
        @tweets = @client.search(@url.split('?').first, :result_type => @result_type).statuses.reject{|i| i.text.include?(@item.title.split.first(5).join(' '))}.reject{|i| i.text[0..1] == 'RT'}
        tweets2 = @client.search(short_url.split('?').first, :result_type => @result_type).statuses.reject{|i| i.text.include?(@item.title.split.first(5).join(' '))}.reject{|i| i.text[0..1] == 'RT'}
        tweets2.each do |tweet|
          @tweets << tweet unless @tweets.detect{|t| t.id == tweet.id}
        end
      end
